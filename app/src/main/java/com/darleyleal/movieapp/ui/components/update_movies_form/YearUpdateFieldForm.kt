...
@Composable
fun YearUpdateFieldForm(
    movieFormViewModel: MovieFormViewModel,
    movie: Movie,
    modifier: Modifier = Modifier
) {
    MovieAppTheme {
        var yearFieldIsValid by remember { mutableStateOf(true) }
        var year by remember { mutableStateOf(movie.year.toString()) }
        movieFormViewModel.yearUpdate = year

        TextField(
            value = year,
            onValueChange = {
                year = it
                yearFieldIsValid = it.trim().isNotEmpty()
            },
            isError = !yearFieldIsValid || movieFormViewModel.yearUpdate.trim().isEmpty(),
            label = {
                Text(text = "Year", color = MaterialTheme.colorScheme.primary)
            },
            trailingIcon = {
                if (!yearFieldIsValid) {
                    Icon(
