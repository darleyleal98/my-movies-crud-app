...
@Composable
fun ImageUpdateLinkFieldForm(
    movieFormViewModel: MovieFormViewModel,
    movie: Movie,
    modifier: Modifier = Modifier
) {
    MovieAppTheme {
        var imageFieldIsValid by remember { mutableStateOf(true) }
        var imageLink by remember { mutableStateOf(movie.image.toString()) }
        movieFormViewModel.imageLinkUpdate = imageLink

        TextField(
            value = imageLink,
            onValueChange = {
                imageLink = it
                imageFieldIsValid = it.trim().isNotEmpty()
            },
            isError = !imageFieldIsValid || movieFormViewModel.imageLinkUpdate.trim().isEmpty(),
            label = {
                Text(text = "Image Link", color = MaterialTheme.colorScheme.primary)
            },
            trailingIcon = {
                if (!imageFieldIsValid) {
                    Icon(
                        Icons.Filled.Error,
                        tint = Color.Red,
